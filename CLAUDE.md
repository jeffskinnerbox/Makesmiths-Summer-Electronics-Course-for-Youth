...
input/my-vision.md  (the Specification — instructor's intent)
        │
        ├─(/syllabus_generator)──────────────▶ lecture_notes/syllabus.md
        │
        ├─(/bill_of_materials_generator)─────▶ BOM  (source input/my-bom.md → improved lecture_notes/BOM.md; the cost/sourcing source of truth)
        │
        └─(/lesson_plan_generator)───────────▶ lecture_notes/class-NN-lecture-notes.md
                                                     │
...
All generated course documents — syllabus, BOM, and the six per-class `class-NN-lecture-notes.md` files
(each holding both the class's lesson plan and its lecture notes) — live together in `lecture_notes/`
(this directory was formerly named `lesson_plans/`, and the per-class files `class-NN-lesson-plan.md`).
...
wherever a non-obvious decision traces back to an answer (see `lecture_notes/syllabus.md` Appendix A
and `lecture_notes/BOM.md` for the established pattern).
...
(analog electronics) course there is no software: the generated syllabus (`lecture_notes/syllabus.md`), the
generated BOM (`lecture_notes/BOM.md`; original source is `input/my-bom.md`), and the per-class
`class-NN-lecture-notes.md` files (each combining the class's lesson plan and lecture notes) all live
together under `lecture_notes/`.
...
- `my-notes.md` — supporting notes; `my-bom.md` — the original BOM source (the improved BOM is generated to `lecture_notes/BOM.md`).
...
pandoc -f gfm lecture_notes/syllabus.md -o syllabus.docx   # export a doc
```
...
`lecture_notes/` (there is no `docs/`; that path applies only to the software-course branch), so read
`@input/*.md` and `@lecture_notes/*.md`.
