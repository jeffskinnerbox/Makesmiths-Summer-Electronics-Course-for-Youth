...
input/my-vision.md  (the Specification — instructor's intent)
        ...
        └─(/lesson_plan_generator)───────────▶ lesson_plans/class-NN-lesson-plan.md
                                                     │
        the lesson plan and the lecture notes for a class share ONE file; these three skills
        append onto the same per-class file:
        (/explainer, /history_and_application, /theory_of_operation) ▶ lesson_plans/class-NN-lesson-plan.md
```

All generated course documents — syllabus, BOM, and the six per-class `class-NN-lesson-plan.md` files
(each holding both the class's lesson plan and its lecture notes) — live together in `lesson_plans/`.
...
generated BOM (`lesson_plans/BOM.md`; original source is `input/my-bom.md`), and the per-class
`class-NN-lesson-plan.md` files (each combining the class's lesson plan and lecture notes) all live
together under `lesson_plans/`.
...
the appendix convention. When the instructor asks for an explainer using *both* `/explainer` and
`/theory_of_operation` (a recurring request — see `the-role-of-feedback.md`, `rc-networks.md`,
`capacitive-coupling.md`, `displacement-current.md`), merge them into **one document** following the
established pattern: an explainer-style hook tied to the class experiment, numbered framing questions, a
"short answer," then the theory-of-operation sections (Overview / Detailed Operation steps / Block Diagram /
Key Parameters / Common Misconceptions), a "why this matters everywhere" narrative, and a "Bottom line" —
match `the-role-of-feedback.md` exactly. New explainers must be added to the question-per-file table in
`explainers/README.md`. Treat all of the above like README — useful context, but exempt from the appendix
...
- `my-claude-prompts.md` — the instructor's prompt playbook (the exact prompts that drive each document;
  read it to understand intended workflow and ordering). The instructor appends each session's prompt as a
  numbered "Nth Claude Code Prompt" entry, so this file is often modified-but-uncommitted; when asked to
  "commit all files," it is expected to be included.
- `Make-Electronics-2nd-Edition.pdf` — **~850 pages, 32 MB. Cannot be read in one call.** Use the Read tool
  ...
  Page anchors found so far (PDF page numbers, not book pages): Exp 9 starts ≈ p. 197 (RC network/time
  constant ≈ 201–207, capacitive coupling ≈ 209–212, displacement current ≈ 213–218), Exp 10 starts ≈ p. 220.
  The per-experiment handouts in `handouts/` also cite the book page ranges they draw from.
...
sure they are consistent and correct" step — for *this* course the generated documents live in
`lesson_plans/` (there is no `docs/`; that path applies only to the software-course branch), so read
`@input/*.md` and `@lesson_plans/*.md`.
