...
**Auxiliary (non-pipeline) documents at the repo root** are hand-written course materials that fall outside
...
code ELL-W101): `cheatsheet.md` (per-class student cheat sheet), `letter-to-parents.md`, `registration.md`
(event/registration details; moved here from `input/`), `electromagic.md` (scratch reference links), and
`readme-file-creation.md` (notes). Treat these like README — useful context, but exempt from the appendix
